...
// HasAssignedGroups applies the HasEdge predicate on the "assigned_groups" edge.
func HasAssignedGroups() predicate.APIKey {
	return predicate.APIKey(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasAssignedGroupsWith applies the HasEdge predicate on the "assigned_groups" edge with a given conditions (other predicates).
func HasAssignedGroupsWith(preds ...predicate.Group) predicate.APIKey {
	return predicate.APIKey(func(s *sql.Selector) {
		step := newAssignedGroupsStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
