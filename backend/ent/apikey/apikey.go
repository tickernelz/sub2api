...
const (
	...
	EdgeGroup = "group"
	// EdgeAssignedGroups holds the string denoting the assigned_groups edge name in mutations.
	EdgeAssignedGroups = "assigned_groups"
	...
	// AssignedGroupsTable is the table that holds the assigned_groups relation/edge. The primary key declared below.
	AssignedGroupsTable = "api_key_groups"
	// AssignedGroupsInverseTable is the table name for the Group entity.
	// It exists in this package in order to avoid circular dependency with the "group" package.
	AssignedGroupsInverseTable = "groups"
...
var (
	// AssignedGroupsPrimaryKey and AssignedGroupsColumn2 are the table columns denoting the
	// primary key for the assigned_groups relation (M2M).
	AssignedGroupsPrimaryKey = []string{"api_key_id", "group_id"}
)
...
// ByAssignedGroupsCount orders the results by assigned_groups count.
func ByAssignedGroupsCount(opts ...sql.OrderTermOption) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborsCount(s, newAssignedGroupsStep(), opts...)
	}
}

// ByAssignedGroups orders the results by assigned_groups terms.
func ByAssignedGroups(term sql.OrderTerm, terms ...sql.OrderTerm) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborTerms(s, newAssignedGroupsStep(), append([]sql.OrderTerm{term}, terms...)...)
	}
}
...
func newAssignedGroupsStep() *sqlgraph.Step {
	return sqlgraph.NewStep(
		sqlgraph.From(Table, FieldID),
		sqlgraph.To(AssignedGroupsInverseTable, FieldID),
		sqlgraph.Edge(sqlgraph.M2M, false, AssignedGroupsTable, AssignedGroupsPrimaryKey...),
	)
}
