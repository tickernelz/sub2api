...
// QueryAssignedGroups chains the current query on the "assigned_groups" edge.
func (_q *APIKeyQuery) QueryAssignedGroups() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withAssignedGroups: _q.withAssignedGroups.Clone(),
...
// WithAssignedGroups tells the query-builder to eager-load the nodes that are connected to
// the "assigned_groups" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithAssignedGroups(opts ...func(*GroupQuery)) *APIKeyQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAssignedGroups = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withAssignedGroups; query != nil {
		if err := _q.loadAssignedGroups(ctx, query, nodes,
			func(n *APIKey) { n.Edges.AssignedGroups = []*Group{} },
			func(n *APIKey, e *Group) { n.Edges.AssignedGroups = append(n.Edges.AssignedGroups, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadAssignedGroups(ctx context.Context, query *GroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(apikey.AssignedGroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(apikey.AssignedGroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(apikey.AssignedGroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(apikey.AssignedGroupsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "assigned_groups" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *APIKeyQuery) loadAPIKeyGroups(ctx context.Context, query *APIKeyGroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *APIKeyGroup)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "api_key_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
