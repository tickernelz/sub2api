...
func defaultModelsListCandidateIDs(platform string) []string {
	...
	return providerregistry.DefaultModelIDsForPlatform(PlatformAnthropic)
}
...
func (s *adminServiceImpl) AdminUpdateAPIKeyGroupID(ctx context.Context, keyID int64, groupID *int64) (*AdminUpdateAPIKeyGroupIDResult, error) {
	...
	if *groupID == 0 {
		...
		gid := *groupID
		assignedGroupIDs := apiKey.GroupIDs
		if len(assignedGroupIDs) == 0 && apiKey.GroupID != nil && *apiKey.GroupID > 0 {
			assignedGroupIDs = []int64{*apiKey.GroupID}
		}
		_, normalizedGroupIDs, err := normalizeAPIKeyGroupAssignments(&gid, append([]int64{gid}, assignedGroupIDs...))
		if err != nil {
			return nil, err
		}
		apiKey.GroupID = &gid
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if len(input.Credentials) > 0 {
		...
		if account.Platform == PlatformOpenAI && account.Type == AccountTypeOAuth && openAICredentialsPayloadHasToken(input.Credentials) {
			applyOpenAIRefreshTokenRecoveredExtra(account)
		}
	}
