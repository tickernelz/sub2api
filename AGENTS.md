...
|---|---|---|
| `e19cd230d` | `fix(openai): keep oauth accounts schedulable on reused refresh token` | Backend soft-handle |
| `abb0d4cc8` | `feat(admin): show OpenAI OAuth reauth warning` | Frontend warning UI |
...
Satu commit sumber (fork-only): `6dc5a4d1a` `feat(gateway): stream stale detection + auto-failover across all providers` (~1900 baris, 20 file). Default enabled (TTFT 60s, gap-warn 10s, gap-timeout 30s).
...
| `frontend/package.json` + `frontend/pnpm-lock.yaml` | ~~Dev dependency langsung fork~~ **RESOLVED 2026-07-20** | Upstream sejak `e625ce3b3` sudah mendeklarasikan `@intlify/message-compiler@9.14.5` langsung; masih benar pada `2730c1c43`. Ikuti upstream exact; jangan re-apply specifier fork `^9.14.5`. |
