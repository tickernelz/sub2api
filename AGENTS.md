...
> ⚠️ **SHA berubah tiap sync.** SHA aktif setelah sync 2026-07-17 di atas upstream `bc50c9d01`: `56bbd6d7a`, `260ee06f2`, `7a6730912`, `85ca7e6d5`, dan `23b450de9`. Setiap reset + cherry-pick menghasilkan SHA baru; cari ulang berdasarkan judul commit dengan `git log --oneline --all --grep=...` (lihat §4).
...
|---|---|---|
| `56bbd6d7a` | `fix(openai): keep oauth accounts schedulable on reused refresh token` | Backend soft-handle |
| `260ee06f2` | `feat(admin): show OpenAI OAuth reauth warning` | Frontend warning UI |
...
Satu commit sumber (fork-only): `7a6730912` `feat(gateway): stream stale detection + auto-failover across all providers` (~1900 baris, 20 file). Default enabled (TTFT 60s, gap-warn 10s, gap-timeout 30s).
...
Dua commit sumber (fork-only): `85ca7e6d5` (neutralizer + dua builder) dan `23b450de9` (observability `invalid_prompt`). Cari ulang dengan `git log --oneline --all --grep="harmony"` atau `--grep="invalid_prompt"`. Default **enabled** (`gateway.neutralize_harmony_channel_token=true`); bisa dimatikan tanpa rebuild.
...
| `frontend/package.json` + `frontend/pnpm-lock.yaml` | Dev dependency langsung `@intlify/message-compiler@^9.14.5` | Upstream masih mengimpor package ini langsung dari `localesMessageCompile.spec.ts`, tetapi sampai base `bc50c9d01` hanya menyediakannya transitif lewat `vue-i18n`; pnpm strict layout tidak membuat root link sehingga full Vitest gagal resolve. Hapus divergence saat upstream mendeklarasikannya sendiri. |
...
#    - declare @intlify/message-compiler langsung jika upstream masih belum melakukannya
...
git add .github .gitignore AGENTS.md backend/cmd/server/VERSION frontend/package.json frontend/pnpm-lock.yaml
