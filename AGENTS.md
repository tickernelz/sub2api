...
> ⚠️ **SHA berubah tiap sync.** SHA aktif setelah sync 2026-07-16 di atas upstream `eb2b8632d`: `85cc132bf`, `aa71f090a`, `dfc0315f5`, `eb07be8b6`, dan `5f329ec63`. Setiap reset + cherry-pick menghasilkan SHA baru; cari ulang berdasarkan judul commit dengan `git log --oneline --all --grep=...` (lihat §4).
...
|---|---|---|
| `85cc132bf` | `fix(openai): keep oauth accounts schedulable on reused refresh token` | Backend soft-handle |
| `aa71f090a` | `feat(admin): show OpenAI OAuth reauth warning` | Frontend warning UI |
...
Satu commit sumber (fork-only): `dfc0315f5` `feat(gateway): stream stale detection + auto-failover across all providers` (~1900 baris, 20 file). Default enabled (TTFT 60s, gap-warn 10s, gap-timeout 30s).
...
Dua commit sumber (fork-only): `eb07be8b6` (neutralizer + dua builder) dan `5f329ec63` (observability `invalid_prompt`). Cari ulang dengan `git log --oneline --all --grep="harmony"` atau `--grep="invalid_prompt"`. Default **enabled** (`gateway.neutralize_harmony_channel_token=true`); bisa dimatikan tanpa rebuild.
...
| `frontend/package.json` + `frontend/pnpm-lock.yaml` | Dev dependency langsung `@intlify/message-compiler@^9.14.5` | Upstream `eb2b8632d` menambah `localesMessageCompile.spec.ts` yang mengimpor package ini langsung, tetapi hanya tersedia transitif lewat `vue-i18n`; pnpm strict layout tidak membuat root link sehingga full Vitest gagal resolve. Hapus divergence saat upstream mendeklarasikannya sendiri. |
