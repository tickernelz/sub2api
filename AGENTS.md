...
> ⚠️ **SHA berubah tiap sync.** SHA aktif setelah sync 2026-07-30 di atas upstream `5a6143097`: `5a5118b97`, `eb65f4426`, `ffefdebab`, `7cacc49ee`, `7c7b36d4f`, dan `5fb0b8f00`. Setiap reset + cherry-pick menghasilkan SHA baru; cari ulang berdasarkan judul commit dengan `git log --oneline --all --grep=...` (lihat §4).
...
|---|---|---|
| `5a5118b97` | `fix(openai): keep oauth accounts schedulable on reused refresh token` | Backend soft-handle |
| `eb65f4426` | `feat(admin): show OpenAI OAuth reauth warning` | Frontend warning UI |
...
Satu commit sumber (fork-only): `ffefdebab` `feat(gateway): stream stale detection + auto-failover across all providers` (1923 insertions, 22 file). Default enabled (TTFT 60s, gap-warn 10s, gap-timeout 30s).
...
**Titik wiring native WebSocket request (commit `5fb0b8f00`):**
...
| `frontend/package.json` + `frontend/pnpm-lock.yaml` | ~~Dev dependency langsung fork~~ **RESOLVED 2026-07-20** | Upstream sejak `e625ce3b3` sudah mendeklarasikan `@intlify/message-compiler@9.14.5` langsung; masih benar pada `5a6143097`. Ikuti upstream exact; jangan re-apply specifier fork `^9.14.5`. |
